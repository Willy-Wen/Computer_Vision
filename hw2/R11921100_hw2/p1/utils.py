...
def get_tiny_images(img_paths):
    ...
    for img_path in img_paths:
        feat = np.array(Image.open(img_path).resize((16, 16))).flatten()
    ...
    #                        END OF YOUR CODE                       #
    #################################################################
    # print(tiny_img_feats.shape) # (1500, 400)
...
###### Step 1-b-1
def build_vocabulary(img_paths, vocab_size=600):
    '''
    Input : 
    ...
    for img_path in tqdm(img_paths):
        ...
        frame, descriptor = dsift(img, step=[STEP, STEP], fast=True)
        features.append(descriptor)
    # print(len(features), features[0].shape) # 1500 (2451, 128)
    features = np.concatenate(features, axis=0).astype('float32')
    # print(features.shape) # (3800991, 128)
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for img_path in tqdm(img_paths):
        img = np.array(Image.open(img_path), dtype='float32')
        frame, descriptor = dsift(img, step=[STEP, STEP], fast=True)
        distance = cdist(vocab, descriptor)
        nearest_index = np.argmin(distance, axis=0)
        # print(nearest_index.shape)
        histogram, bin_edge = np.histogram(nearest_index, bins=len(vocab))
        histogram_normalized = [float(i)/np.sum(histogram) for i in histogram]
        img_feats.append(histogram_normalized)
    img_feats = np.array(img_feats)
    ############################################################################
    #                                END OF YOUR CODE                          #
    ############################################################################
    # print(img_feats.shape) # (1500, 400)
    return img_feats
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    # print(train_img_feats.shape) # (1500, 400)
    # print(test_img_feats.shape) # (1500, 400)
    # print(len(train_labels)) # 1500
    k_nearest = np.argsort(distances, axis=1)[:, :K]
    # print(k_nearest.shape) # (1500, 9)
    for i in range(k_nearest.shape[0]):
        vote = np.array([CAT2ID[train_labels[j]] for j in k_nearest[i]])
        predict_id = np.argmax(np.bincount(vote))
        # print(predict_id)
        test_predicts.append(CAT[predict_id])
    ###########################################################################
    #                               END OF YOUR CODE                          #
    ###########################################################################
    # print(test_predicts)
