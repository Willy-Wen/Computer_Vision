...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    # is not fixed.                                                #
    ################################################################
    loss_fig_path = os.path.join(logfile_dir, 'loss.png')
    plt.title('Loss')
    plt.plot(np.arange(1, len(result_lists['train_loss'])+1), result_lists['train_loss'], label='train_loss')
    plt.plot(np.arange(1, len(result_lists['val_loss'])+1), result_lists['val_loss'], label='val_loss')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
    ...
    acc_fig_path = os.path.join(logfile_dir, 'acc.png')
    plt.title('Accuracy')
    plt.plot(np.arange(1, len(result_lists['train_acc'])+1), result_lists['train_acc'], label='train_acc')
    plt.plot(np.arange(1, len(result_lists['val_acc'])+1), result_lists['val_acc'], label='val_acc')
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.legend()
...
def train(model, train_loader, val_loader, logfile_dir, model_save_dir, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch, data in enumerate(train_loader):
            ...
            pred = model(images)
            # Calculate loss.
            # print(torch.sum(torch.argmax(pred, dim=1) == labels)/128)
            ...
            batch_acc = (torch.sum(torch.argmax(pred, dim=1) == labels)/len(labels)).item()
            sys.stdout.write(f'\r[{epoch + 1}/{cfg.epochs}] Train batch: {batch + 1} / {len(train_loader)}, acc: {batch_acc}')
            sys.stdout.flush()
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                ...
                batch_acc = (torch.sum(torch.argmax(pred, dim=1) == labels) / len(labels)).item()
                sys.stdout.write(
                    f'\r[{epoch + 1}/{cfg.epochs}] Val batch: {batch + 1} / {len(train_loader)}, acc: {batch_acc}')
                sys.stdout.flush()
            ######################### TODO End ##########################
        ...
        current_result_lists = {
            'train_acc': torch.tensor(train_acc_list, device='cpu').numpy(),
            'train_loss': torch.tensor(train_loss_list, device='cpu').numpy(),
            'val_acc': torch.tensor(val_acc_list, device='cpu').numpy(),
            'val_loss': torch.tensor(val_loss_list, device='cpu').numpy()
        }
